"""Mapper from raw GCP BigQuery billing responses to provider-agnostic schemas.

:class:`GCPMapper` is a stateless adapter that turns the normalized dict
returned by :class:`app.services.gcp.billing.GCPBillingService` into
the :class:`app.providers.schemas.CostResponse` model that the rest of
the application consumes. Keeping this translation in its own class makes
the mapping logic easy to unit test in isolation from the BigQuery SDK.
"""
...
class GCPMapper:
    """Translate GCP-specific dicts into :class:`CostResponse`."""
    ...
    def map(
        ...
        """Convert a raw GCP cost dict into a :class:`CostResponse`.

        The ``raw`` dict is the normalized structure produced by
        :meth:`app.services.gcp.billing.GCPBillingService.get_costs`
        ...
        return CostResponse(
            provider=str(raw.get("provider", "gcp")),
