...
Cost Explorer. It supports 7, 14, and 30-day horizons and total, provider,
service, account, and region dimensions where the source data supports them.
Forecast values are generated by deterministic statistical logic and are not
generated by an LLM.

The baseline is a weighted average of the seven most recent daily observations
combined with a bounded linear trend. Missing calendar dates are represented as
zero-cost observations and reported in data quality metadata. Duplicate records
are removed deterministically. A robust median/MAD filter identifies extreme
recent observations similar to Sprint 1.7 anomalies and excludes them from the
baseline when sufficient history remains; excluded dates remain visible to the
user. The service returns expected daily values, lower/upper confidence bounds,
confidence level, comparable historical spend, and the methodology version.

At least seven daily observations are required. Empty and sparse histories return
structured `empty` or `insufficient_data` responses without fabricated values.
Forecast cache keys include user scope, every query input, and the methodology
version, so algorithm changes invalidate old results. Each daily forecast retains
its date and expected value, allowing completed periods to be evaluated later
with deterministic MAE and MAPE calculations when actuals are available.
